...
$approveImage = "";
	$deleteImage = "";
	$filesForApproval = "";
	$filesForApprovalArray = "";

		if (isset($_POST['approveImage'])){

			$approveImage = $_POST['approveImage'];

			if (empty($_POST['filesForApproval'])){
				$filesForApproval =  "";
			} else {
				$filesForApproval = test_input($_POST['filesForApproval']);
				$filesForApprovalArray= explode(',', $filesForApproval);
			}
		...
		echo "delete image: " .$deleteImage;
		echo "<br>";
		echo "approve image: " .$approveImage;
		echo "<br>";
		echo "value: " .$filesForApproval;
		print_r($filesForApprovalArray);
			...
			foreach($phparray as $key => $value) {
				foreach ($filesForApprovalArray  as $value2) {
					if($value['imagefile'] === $value2) {
			    		...
			    		$number = preg_replace('/[^0-9]/', '', $value['imagefile']);
			    		unlink($value['imagefile']);
						unlink("thumbnails/thumb".$number. ".jpg");
						array_splice($phparray, $key,1);
			   		}
	...
	function displayUnapprovedImages(){
	$jsonstring = "";
		 $files = glob("uploads/*.*");
		 $thumbs = glob("thumbnails/*.*");
